...
public Plugin:myinfo = 
{
	name = "Show Damage",
	author = AUTHOR,
	description = "This plugin will show damages you deal and the health you left to your ennemies",
...
public OnPluginStart()
{
	...
	CreateConVar("sm_sdh_version", PLUGIN_VERSION, "Version of Show Damage Center.", FCVAR_NOTIFY);
	pluginEnabled = CreateConVar("sm_sdh_enable", "1", "(1)Enable or (0)Disable Show Damage Center. Default: 1", FCVAR_NOTIFY);
	RegConsoleCmd("sdh", ShowDamageCommand);
	showDamageCookie = RegClientCookie("showDamage", "Show Damage Center (0)On/(1)Off", CookieAccess_Public);
}
...
public Action:ShowDamageCommand(client, args)
{
	if (GetConVarInt(pluginEnabled) == 1)
	{
		...
		{
			PrintToChat(client, "\x01\x0B\x05[SD]\x01 Type \x05!sdh\x01 to Enable or Disable Show Damage Center");
		}
